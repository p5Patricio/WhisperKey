/**
 * Threads WebGL Background Component (React Bits port to Vanilla JS)
 * Uses OGL for high-performance perlin-noise wave line rendering.
 */
...
export function initThreads(container, options = {}) {
  ...
  const config = {
    color: options.color || [0.25, 0.85, 0.75], // Teal/cyan tech accent
    amplitude: options.amplitude !== undefined ? options.amplitude : 2.4,
  ...
  try {
    ...
    console.warn('WebGL init failed for Threads:', err);
    return null;
  }
  ...
  return {
    ...
    setColor(rgb) {
      config.color = rgb;
      program.uniforms.uColor.value.set(...rgb);
    }
  };
}
